...
countries.stream().map(country -> country.getName()).filter(country -> country.toLowerCase().contains("island")).findFirst().get()
...
countries.stream().sorted(Comparator.comparingLong(Country::getPopulation)).map(country -> country.getName()).limit(10).forEach(System.out::println)
...
countries.stream().sorted(Comparator.comparingInt(country -> country.getTimezones().size())).forEach(country -> System.out.println( country.getName() + "_:_" + country.getPopulation()))
...
countries.stream().filter(country -> !(country.getArea() == null)).filter(country -> country.getArea().compareTo(BigDecimal.TEN) < 0).map(Country::getName).forEach(System.out::println)
